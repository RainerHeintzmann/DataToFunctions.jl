...
```
"""
function get_function(data::AbstractArray; super_sampling=2, extrapolation_bc=Flat(), interp_type=Interpolations.BSpline(Linear()))
    new_size = super_sampling.*size(data)
    upsampled = resample(data, new_size)
    # @show upsampled
    # return upsampled
    ...
    function zoomed(shift, zoom)
        ...
        center_upsamp = ((center_orig .-1) .*super_sampling .+1)  # new_size .÷2 .+1
