...
export get_interpolated_function, get_function_tuple, get_function_svec, get_function_affine
...
@inline@inline function add_dim(v::NTuple{N,T}) where {N,T}
    SVector{N + 1,T}(v..., one(T))
end


@inline function red_dim(v::StaticVector{N,T}) where {N,T}
    last_inv = inv(v[N])
...
@inline function transform_and_sample(itp, coord_transf_func, I)
    coords = coord_transf_func(Tuple(I))
    return idx_apply(itp, coords)
end
...
# end
function apply_transform(coord_transf_func, data, itp)
    inds = CartesianIndices(axes(data))

    sample(I) = transform_and_sample(
        itp,
        coord_transf_func,
        I,
    )
...
#     #return out
# end
@inline mat_mul(v, mymat) = mymat * v


function apply_transform_homogen(
    coord_transf_func,
    data,
    itp,
)
...
@inline function poly_sample(itp, poly, coeffs, I)
...
"""
    get_function_poly(data, ::Val{N_order})

Returns a function which warps `data` using a polynomial coordinate
transformation of order `N_order`.

The returned function is non-mutating and AD-compatible.
Its forward evaluation allocates only the returned output array.
"""
function get_function_poly(
    data::AbstractArray{T,N},
    ::Val{N_order};
    extrapolation = zero(T),
) where {T,N,N_order}

    itp = extrapolate(
        interpolate(data, BSpline(Linear())),
        extrapolation,
    )
...
"""
    get_function_poly_inplace(data, ::Val{N_order})

Returns an in-place polynomial warp function

    f!(out, coeffs)

which writes the warped image into `out`.
"""
